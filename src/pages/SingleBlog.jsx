...
const SingleBlog = () => {
        ...
        blog?.map((val) => (
          ...
          <div key={val?.id} className="w-[70%] max-lg:w-full flex flex-col gap-[30px] ">
            <div
              ...
              className="w-full  max-h-[630px] flex flex-col items-center    rounded-2xl overflow-hidden"
            >
              <h1 className="text-white text-[clamp(20px,5.2vw,50px)] text-center single_blog_heading ">{val?.title}</h1>
              <img
            ...
            <div className="w-full flex flex-col items-center justify-center ">
                ...
                { btn1 && blog && blog[0] && <button onClick={handleBtn1} className = {`btn  ${!timmer2 && "btn-neutral"} h-fit p-2  ${timmer2 && "bg-transparent rounded-full text-[#feb51f] "}  my-3`} >
              ...
              >
               ...
               {code1 &&  <span className="text-xl text-[#feb51f] text-center" >{code1}</span>  }
            ...
            <div className="content" dangerouslySetInnerHTML={{ __html: val?.content }} />
            <div className="flex flex-col items-center gap-[10px]" >
            {blog && blog[0] && <button  onClick={handleBtn2} className = {` w-fit mx-auto btn  h-fit p-2 ${!timmer1 &&  "btn-neutral"}  ${timmer1 && "bg-transparent outline-none rounded-full text-[#feb51f] "}  `} >  {
            timmer1 ?
            <CountdownCircleTimer
    isPlaying
    strokeWidth={3}
  ...
  >
    {({ remainingTime }) => remainingTime}
        ...
        : "Generate Code"   }</button>}
            {code2 && <span className="text-xl text-[#feb51f] text-center" >{code2}</span>}
