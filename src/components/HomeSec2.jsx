...
const HomeSec2 = () => {
  ...
  return (
    <div ref={containerRef} className='w-full overflow-x-hidden relative max-md:min-h-fit min-h-screen max-md:py-[10px] py-[40px]  flex justify-center items-center' >
        ...
        <div className='flex absolute max-md:top-28 max-sm:top-10 top-48 max-md:gap-[20px] gap-[100px]' >
            {
                bgImages && bgImages?.map((val, index) =>(
                    ...
                    <img loading="lazy" width="500" height="300" ref={(el) => (rightToLeftRef.current[index] = el)} key={val?.id}  src={val?.value} className='w-[clamp(40px,4.3vw,100px)]  max-sm:w-[30px]' alt="logo" />
                ))
            }
        </div>
        ...
        <div className='flex absolute max-md:bottom-28 max-sm:bottom-10  max-sm:gap-[20px]  bottom-56 gap-[100px]' >
        {
  bgImages && bgImages?.reverse().map((val, index) => (
    ...
    loading="lazy" width="500" height="300"
      ...
      key={val?.id}
      src={val?.value}
      className="w-[clamp(40px,4vw,100px)] max-sm:w-[30px] h-auto"
      alt="logo"
    />
  ))
}
